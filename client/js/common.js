...
function isDomainNamePrivate(str) 
{	
	var pat = /^[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,4}$/g;
	var vr = str;
	var vr = vr.replace(pat,"");
	if (vr) { return false; }
...
function isSubWebURL(str) 
{
 ...
 // are regular expressions supported?
  ...
  if (!supported) 
	...
	//var r1 = new RegExp("^(http(s)?):\/\/+([a-zA-Z0-9-_].)+[a-zA-Z0-9\\-\\.]{2,}\\.[a-zA-Z]{2,}$");	  	
...
function CompareDatesTimeChk(startDate,endDate) 
{ 
	var startDateVal = startDate.split(" ");
	var endDateVal = endDate.split(" ");
	
	var str1 = startDateVal[0];
	var str2 = endDateVal[0];
    ...
    { 
		...
		if(startDateVal[2]=='am' && startTime[0]==12)
		{
			startHours = 0;	
		}
		if(endDateVal[2]=='am' && endTime[0]==12)
		{
			endHours = 0;	
		}
	
		if(startDateVal[2]=='pm' && startTime[0]!=12)
		{
			startHours = parseInt(startTime[0])+12;
		}
	
		if(endDateVal[2]=='pm' && endTime[0]!=12)
		{	
			endHours = parseInt(endTime[0])+12;
		}
		
		if(startTime[0]==12 && startDateVal[2]=='am' && endTime[0]==12 && endDateVal[2]=='pm')
		{
				return '1';
		}
		else if(endDateVal[0]==startDateVal[0] && startHours>endHours)
		{
        	return '0';
		}
		else
		{
			if(endDateVal[0]==startDateVal[0] && startHours==endHours && startTime[1]>=endTime[1])
			{
				return '0';
			}
			else
...
function enterData_Without_Space(e, ctrl) {

<?php
	$objClientAdmin->user_id = $cmn->setVal(trim($cmn->readValueSubmission($objEncDec->decrypt($_POST["hdnuser_id"]),"")));
	...
	$objClientAdmin->user_isactive = $cmn->setVal(trim($cmn->readValueSubmission($_POST["rdouser_isactive"],"")));
	$confirmPassword = $cmn->setVal(trim($cmn->readValueSubmission($_POST["user_confirmpassword"],"")));
	
	$objClientAdmin->created_by = $cmn->getSession(ADMIN_USER_ID);

	...
	if(isset($_REQUEST['searchname']))
{
	...
	if($regdateFrom && $regdateTo=='')
		$condition .= " and voting_date>=".$regdateFrom;
	else if($regdateTo && $regdateFrom=='')
		$condition .= " and voting_date<= ".$regdateTo;
	else if($regdateTo!='' && $regdateFrom!='')
		$condition .= " and voting_date between '".$regdateFrom."' and '".$regdateTo."'";
	if($page>0)
	{
...
{
	$Error = $doc->createElement("Error");
	$Error->appendChild($doc->createTextNode("No record Found."));
	$EIResponse->appendChild($Error);
	echo $doc->saveXML();
	exit;
}

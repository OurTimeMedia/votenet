<?php 
	header('Content-Type: application/x-msexcel; name=HitsReport.xls');
	header('Content-Disposition: inline;filename=HitsReport.xls');
	...
	$pagedata = $objpaging->setPageDetails($objTopClient,"reports_state_by_state_summary.php",PAGESIZE,$condition);
	...
	$first = true;
	$str1 = "Page Name^^^Total Hits###";
	for ($i=0;$i<count($pagedata);$i++)
	{
		 if($cmn->readValueDetail($pagedata[$i]['page_name'])=='') 
		 $str1.= "Home";
		 else
		 $str1.= $cmn->readValueDetail($pagedata[$i]['page_name']);
		$str1.="^^^".$cmn->readValueDetail($pagedata[$i]['tot_cnt'])."###";
	//echo $str1;exit;
	}

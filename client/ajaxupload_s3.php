...
require_once SERVER_ROOT.'common/class/s3/s3.php';
	...
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null, $namefile){
		$folder = $relPath;
		$maxlimit = $maxSize;
			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					if($ext==end($file_ext)){
						...
						if(isset($_REQUEST['output']) && $_REQUEST['output'] == "jpg")
							$newfilename = $namefile.".jpg";
						else
							$newfilename = $namefile.".".end($file_ext);
						...
						if(!file_exists($save))
						{
							...
							if(isset($_REQUEST['output']) && $_REQUEST['output'] == "jpg")
							{
								...
								$filename_s3 = strstr($save, 'ElectionImpactProd/files/');
								//$s3 = new s3(AMAZON_KEY, AMAZON_SECRET_KEY);
								//$s3->putObject(BUCKET_NAME, $filename_s3, $save, $_FILES[$fileName]['type'], sprintf("%u", filesize($save)), 'public-read', array(), "");
								unlink($save);
							}
							else
							{
	...
	$namefile = strip_tags($_REQUEST['namefile']);

	if($filesize_image > 0){
		$upload_image = uploadImage($filename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH, $namefile);
...
<?php
	...
	echo 'Image Uploaded';
		...
		echo substr(trim($error_message), 0, -1);
	}
?>
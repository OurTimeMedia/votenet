<?php
	...
	if ( isset($_POST['btnsubmit'])){
		...
		$objvalidation->add_validation("txttitle", "title", "dupli", "", DB_PREFIX."testimonial|title|id|".$id."|1|1");
		$objvalidation->add_validation("txtcontent", "content", "req");
		
		if($_FILES['txt_file']['name'] != '')
			$objvalidation->add_validation("txt_file", "Image", "ext","","jpg,jpeg,png,bmp,gif");
		...
		if($objvalidation->validate())
		{
			...
			{
				$obj->add();
				$msg->send_msg('testimonial-list.php','Customer ',3);
			}
			...
			{
				$obj->update();
				$msg->send_msg('testimonial-list.php','Customer ',4);
			}
		}
	}
	...
	if (isset($_POST['hdnmode']) && trim($_POST['hdnmode']) == 'delete')
	{
		...
		{
			$msg->send_msg('testimonial-list.php','Customer ',9);
		}
		else
		{
			$obj->checkedids = implode(',',$_POST['deletedids']);
			$obj->delete();
			$msg->send_msg('testimonial-list.php','Customer ',5);
		}
		exit();
	}
	...
	if (isset($_POST['hdnmode']) && trim($_POST['hdnmode']) == 'active')
	{
		...
		$obj->activeinactive();
		$msg->send_msg('testimonial-list.php','Customer ',15);
		exit();
	}
	...
	{
		$cmn->export_to_csv('testimonial', 'customers.csv');
	}
	
	//Code to delete document
	if(isset($_POST['hdnmodedeletetestimonial']))
	{
        $obj->deleteFiles((int)$_POST['hdnmodedeletetestimonial']);
		$obj->updateImage((int)$_POST['hdnmodedeletetestimonial']);
	}
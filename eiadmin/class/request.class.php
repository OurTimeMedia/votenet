...
class request
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'request WHERE 1=1 ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'request WHERE 1=1 ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function add()
	{
		$strquery='INSERT INTO '.DB_PREFIX.'request SET
					title			= \''.$this->title.'\',
	...
	function update()
	{
		$strquery='UPDATE '.DB_PREFIX.'request SET 
					title			= \''.$this->title.'\',
					first_name		= \''.$this->first_name.'\',
					last_name		= \''.$this->last_name.'\',
					organization	= \''.$this->organization.'\',
	...
	function delete()
	{
		$strquery='DELETE FROM '.DB_PREFIX.'request  WHERE id in('.$this->checkedids.')';
		return mysql_query($strquery) or die(mysql_error());
	}
	...
	{
		$strquery	=	'UPDATE ' . DB_PREFIX . 'request SET active=\'n\',modified_date	= \''.time().'\' WHERE id in(' . $this->uncheckedids . ')';
		$result = mysql_query($strquery) or die(mysql_error());
		if($result == false)
			return ;
		$strquery	=	'UPDATE ' . DB_PREFIX . 'request SET active=\'y\',modified_date	= \''.time().'\' WHERE id in(' . $this->checkedids . ')';
		return mysql_query($strquery) or die(mysql_error());
	}
}
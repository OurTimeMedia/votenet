...
{	
	...
	if (actiontype=="delete")
	{
		...
		if(confirm("Are you sure you want to delete selected Client Admin(s)?"))
		{
...
function validate()
{
	...
	arValidate[index++] = new Array("S", "document.frm.selPlan", "Plan");
	
	if (!Isvalid(arValidate)){
		return false;
	}
	
	if(document.getElementById('currentplan').value != 0 && document.getElementById('currentplan').value != document.getElementById('selPlan').value)
	{
		var res=confirm("Are you sure to change the client plan? This will set client payment status to Pending.");
	
		if(!res)
			return false;
	}
	
	if(document.getElementById('currentExpiry').value != "" && document.getElementById('currentExpiry').value != document.getElementById('txtexpiry_date').value)
	{
		var res=confirm("Are you sure to change the expiry date? This will set client payment status to Pending.");
	...
	var index = 0;
	var arValidate = new Array;
	arValidate[index++] = new Array("R", "document.frm.txtexpiry_date", "Expiry Date");
	//arValidate[index++] = new Array("R", "document.frm.rdoallow_credit", "Allow Credit");
	arValidate[index++] = new Array("S", "document.frm.rdoallow_credit", "Allow Credit");
			
	//arValidate[index++] = new Array("R", "document.frm.rdouser_isactive", "Status");
	arValidate[index++] = new Array("S", "document.frm.rdouser_isactive", "Status");

...
if ( isset($_REQUEST['btnLayout']))
{
	...
	{	
		...
		$msg->sendMsg("admin_form_builder.php","Layout ",4);
		exit;
	}	
}
...
if ( isset($_REQUEST['btnFormSubmit']))
{
	...
	if ($objvalidation->validate())
	{	
		include("admin_form_builder_field.php");
			...
			{	
				...
				while($inner_cntr < count($language))
				{
					if($inner_cntr == 0)
					{
						...
						$objfield_option->created_by = $cmn->getSession(SYSTEM_ADMIN_USER_ID);
						$objfield_option->updated_by = $cmn->getSession(SYSTEM_ADMIN_USER_ID);	
		...
		if(isset($_POST['condition']) && $_POST['condition'] != "")
		{
			...
			$objfield_condition->created_by = $cmn->getSession(SYSTEM_ADMIN_USER_ID);
			$objfield_condition->updated_by = $cmn->getSession(SYSTEM_ADMIN_USER_ID);	
		...
		{
			$msg->sendMsg("admin_form_builder.php","Field ",3);
			exit;
		}
		...
		{
			$msg->sendMsg("admin_form_builder.php","Field ",4);
			exit;
		}
	}	
}
if(isset($_REQUEST['mode']) && $_REQUEST['mode'] == "delete")
{
	...
	$msg->sendMsg("admin_form_builder.php","Field ",5);
	exit;
}	
?>
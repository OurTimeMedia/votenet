...
class topclientreport
{
	...
	{
		 $sql="SELECT sum( tot_cnt_website ) AS tot_cnt_website,sum( tot_cnt_facebook ) AS tot_cnt_facebook,sum(tot_cnt_gadget) AS tot_cnt_gadget,sum(tot_cnt_mobile) AS tot_cnt_mobile
	...
	//	$sql .= " GROUP BY client_id limit 0,10";
		...
		while($arrpt= mysql_fetch_array($rs))
		{
			...
			$arrlist[$i]["tot_cnt_facebook"] = $arrpt["tot_cnt_facebook"];
			$arrlist[$i]["tot_cnt_gadget"] = $arrpt["tot_cnt_gadget"];
			$arrlist[$i]["tot_cnt_mobile"] = $arrpt["tot_cnt_mobile"];
			//$arrlist[$i]["tot_cnt_api"] = $arrpt["tot_cnt_api"];	
		}	
		return $arrlist;
	}
	...
	{
		 $sql="SELECT * ,sum(tot_cnt_website) as tot_cnt_website, sum(tot_cnt_facebook) as tot_cnt_facebook, sum(tot_cnt_gadget) as tot_cnt_gadget, sum(tot_cnt_api) as tot_cnt_api, sum(tot_cnt_mobile) as tot_cnt_mobile,sum( tot_registration ) AS tot_cnt
		FROM ".REPORT_DB_PREFIX."rpt_date_source_reg rds
		left join ".REPORT_DB_PREFIX."rpt_client rc on rc.client_id=rds.client_id
		where 1=1 ";
		if($fromdate && $todate=='')
		...
		if($order!='')
			...
			$sql .= " order by rds.reg_date desc";
	...
	function registrantdetail($id="",$condition="",$order="voter_email",$fields='*', $limit = "")
	{
	...
	function registrantdetailcount($id="",$condition="")
	{
		$sql="select count(rpt_reg_id) as tot_reg from ".REPORT_DB_PREFIX."rpt_registration
		left join ".REPORT_DB_PREFIX."rpt_state on ".REPORT_DB_PREFIX."rpt_state.state_id=".REPORT_DB_PREFIX."rpt_registration.voter_state_id
		where 1=1";
		$sql .= $condition;
	...
	function getAllStateByStateReportMap($client_id)
	{
		$sql="select sum(rsr.tot_cnt) as tot_cnt,s.state_name,s.state_code from ".REPORT_DB_PREFIX."rpt_state as s left join ".REPORT_DB_PREFIX."rpt_state_reg rsr on s.state_id = rsr.state_id  and rsr.client_id = '".$client_id."' where 1=1 ";		
		$sql .=" group by s.state_id";
		$sql .=" order by s.state_code";
		//echo $sql;
		return mysql_query($sql);
	...
	function fetchFields($client_id=0)
	{
		...
		if(mysql_num_rows($rs) > 0)
		{
			while($arrpt= mysql_fetch_array($rs))
			{
				...
				where 1=1 and (client_id = ".$client_id." or client_id=0) and field_header_field = '".$arrpt['rpt_field_id']."' and field_caption <> '' and field_mapping_id <> '10' and field_isactive=1 and reg_type='General' ORDER BY field_order ";	
				...
				if(mysql_num_rows($rs1) > 0)
				{
					while($arrpt1= mysql_fetch_array($rs1))
					{
						...
						$arrlist[$i]['field_caption']="<strong>".$arrpt['field_caption']."</strong> &raquo; ".$arrpt1['field_caption'];
						$arrlist[$i]['field_order']=$arrpt1['field_order'];
						$arrlist[$i]['field_header_id']=$arrpt['rpt_field_id'];
						$i++;
					}
				}
			}

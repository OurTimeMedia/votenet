<?php
class registration_deadline extends common
{
	var $registration_deadline_id;
	var $state_id;
	var $registration_deadline_notes;
	
		
	function registration_deadline()
	{
		$this->registration_deadline_id = 0;
		$this->state_id = 0;
		$this->registration_deadline_notes = 0;		

	}
	function fetchAllAsArray($id = "",$condition = "",$order = "state_id")
	{		
		$sQuery = "SELECT * FROM ".DB_PREFIX."registration_deadline WHERE 1 = 1 " . $condition ." order by  ". $order;
		...
		{
		$arrlist[$i]["registration_deadline_id"] = $areligibility["registration_deadline_id"];
		$arrlist[$i]["state_id"] = $areligibility["state_id"];
		$arrlist[$i]["registration_deadline_notes"] = $areligibility["registration_deadline_notes"];
		$i++;
		}
		return $arrlist;
	}
	...
	function updateEligibilityCriteria($post)
	{		
	...
	function createLanguageDetailForEligibilityCriteria($language_id)
	{
		...
		{
			echo $strQuery="INSERT INTO ".DB_PREFIX."eligibility_criteria_language 
		...
		{
			echo $strQuery="UPDATE ".DB_PREFIX."eligibility_criteria_language SET
					eligibility_criteria='".$this->eligibility_criteria."', 
					updated_by='".$this->updated_by."', 
					updated_date='".currentScriptDate()."' 
